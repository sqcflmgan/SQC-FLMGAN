...
class Experiment():
    '''Class that contains logic to store hyperparameters und results of an experiment类，包含存储超参数和实验结果的逻辑'''
    hyperparameters = {}
    results = {}
    parameters = {}
...
def get_all_hp_combinations(hp):
    '''Turns a dict of lists into a list of dicts将列表的字典转换为字典的列表'''
...
def list_of_dicts_to_dict(hp_dicts):
    '''Turns a list of dicts into one dict of lists containing all individual values将字典列表转换为包含所有单独值的列表的一个字典'''
    one_dict = {}
    for hp in hp_dicts:
...
def get_list_of_experiments(path, only_finished=False, verbose=True):
    '''Returns all the results saved at location path返回保存在位置路径下的所有结果'''
    list_of_experiments = []
...
def get_experiment(path, name, verbose=False):
    '''Returns one result saved at location path返回保存在位置路径下的一个结果'''
